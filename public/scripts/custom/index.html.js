...
// 		}
    ...
    var html="<div class='module  clearfix'><div class='module-head'>  <h3>Appointment Calendar</h3>      </div>    <div class='module-body clearfix'>        <div id='container_cal'></div>          <div class='clearfix'></div>        </div>    </div>";
    $('#container').html(html);

    $('#container_cal').fullCalendar({
        editable: true,
        events: result
    });
...
function init(){
    ...
    setTimeout(function(){$('#remainderAlerts').hide('slow');},5000);
            ...
            if(typeof data != 'undefined'){
                ...
                }else
                {
                    ...
                    $('.search_menu').click(function(){
                            ...
                            $.getJSON('/update_profile?doc_id='+doc_id,function(data){
...
function loadActivity(){
        ...
        $.getJSON('/data',function(data){
            ...
            if(typeof data != 'undefined' && typeof data.response != 'undefined'){
                ...
                $.getJSON('/getVAHistory',function(data){

                    if(typeof data != 'undefined' && typeof data.response != 'undefined'){

                        var oldData=data;

                        console.log("diff: "+json_data.response['HEALTH CARE PROVIDERS']['Providers'].length!=oldData.response['HEALTH CARE PROVIDERS']['Providers'].length);


                        if(json_data.response['HEALTH CARE PROVIDERS']['Providers'].length!=oldData.response['HEALTH CARE PROVIDERS']['Providers'].length){
                            $('#mnuHealthCareProviders').children(':first-child').children(':first-child').next().next().html(json_data.response['HEALTH CARE PROVIDERS']['Providers'].length-oldData.response['HEALTH CARE PROVIDERS']['Providers'].length);
                            $('#mnuHealthCareProviders').children(':first-child').children(':first-child').next().next().show();
                        }

                        if(json_data.response['TREATMENT FACILITIES']['Facilities'].length!=oldData.response['TREATMENT FACILITIES']['Facilities'].length){
                            $('#mnuTF').children(':first-child').children(':first-child').next().next().html(json_data.response['TREATMENT FACILITIES']['Facilities'].length-oldData.response['TREATMENT FACILITIES']['Facilities'].length);
                            $('#mnuTF').children(':first-child').children(':first-child').next().next().show();
                        }
                        ...
                        if(json_data.response['HEALTH INSURANCE']['Companies'].length!=oldData.response['HEALTH INSURANCE']['Companies'].length){
                            $('#mnuHI').children(':first-child').children(':first-child').next().next().html(json_data.response['HEALTH INSURANCE']['Companies'].length!=oldData.response['HEALTH INSURANCE']['Companies'].length);
                            $('#mnuHI').children(':first-child').children(':first-child').next().next();
                        }


                        if(json_data.response['VA ALLERGIES']['Allergies'].length+json_data.response['ALLERGIES/ADVERSE REACTIONS']['Allergies'].length!=oldData.response['VA ALLERGIES']['Allergies'].length+oldData.response['ALLERGIES/ADVERSE REACTIONS']['Allergies'].length){
                            $('#mnuAL').children(':first-child').next().next().html(json_data.response['VA ALLERGIES']['Allergies'].length+json_data.response['ALLERGIES/ADVERSE REACTIONS']['Allergies'].length-oldData.response['VA ALLERGIES']['Allergies'].length+oldData.response['ALLERGIES/ADVERSE REACTIONS']['Allergies'].length);
                            $('#mnuAL').children(':first-child').next().next();
                        }

                        if(json_data.response['IMMUNIZATIONS']['Immunizations'].length!=oldData.response['IMMUNIZATIONS']['Immunizations'].length){
                            $('#mnuIMMU').children(':first-child').children(':first-child').next().next().html(json_data.response['IMMUNIZATIONS']['Immunizations'].length-oldData.response['IMMUNIZATIONS']['Immunizations'].length);
                            $('#mnuIMMU').children(':first-child').children(':first-child').next().next();
                        }
